...
export function SlugResultsView({ slug }: Props) {
  ...
  return (
    <main className="min-h-screen bg-parchment-100">
        ...
        <div className="grid lg:grid-cols-12 gap-8 lg:gap-16 items-start">
          <div className="lg:col-span-6">
            <h1 className="font-display text-display-xl text-ink-900 mb-6">
              <span className="block text-ink-500 text-2xl font-light mb-3">Total score</span>
      ...
      <div className="max-w-canvas mx-auto px-6 lg:px-12"><div className="h-px bg-ink-900/10" /></div>
            ...
            return (
              <div key={p.id} className="py-6 grid grid-cols-12 gap-4 items-baseline">
                <div className="col-span-7 lg:col-span-6">
                  <h3 className="font-display text-xl text-ink-900 mb-1">{pillar.name}</h3>
                  <p className="text-caption">Weight {Math.round(pillar.weight * 100)}%</p>
                </div>
                <div className="col-span-3 lg:col-span-4">
                  <div className="h-1 bg-ink-900/10 rounded-full overflow-hidden">
                    <div className="h-full bg-ember-600" style={{ width: `${p.percentageScore}%` }} />
                  </div>
                </div>
                <div className="col-span-2 text-right">
                  <span className="font-display text-2xl text-ink-900 tabular-nums">{p.percentageScore}</span>
                </div>
              </div>
            );
          })}
      ...
      <section className="px-6 lg:px-12 max-w-canvas mx-auto py-16">
        ...
        <h2 className="font-display text-display-md text-ink-900 mb-3">Your highest-leverage fixes.</h2>
        <p className="text-ink-600 max-w-prose mb-12 leading-relaxed">
          Ranked by where the biggest gains are. Each comes with a specific first step you can take this week.
        </p>
        <div className="grid gap-6">
          {result.topPriorities.map((p, i) => {
            const pillar = scorecard.pillars.find((pl) => pl.id === p.id)!;
            return <PriorityCard key={p.id} rank={i + 1} pillarScore={p} pillar={pillar} />;
          })}
        </div>
      </section>
      ...
      <section className="px-6 lg:px-12 max-w-canvas mx-auto py-16">
        <div className="max-w-2xl">
          ...
          <h2 className="font-display text-display-lg text-ink-900 mb-6">Want a 30-minute walkthrough?</h2>
          <p className="text-lg text-ink-600 leading-relaxed mb-8">
            The teams that act on a scorecard like this within 30 days typically move up one tier within 6 months.
            Book a free session to discuss your specific report and find the single highest-leverage fix for your stage.
          </p>
          <a href="mailto:abhishek.k0420@gmail.com?subject=RevOps%20Scorecard%20walkthrough" className="btn-primary">
            Book a 30-minute walkthrough
            <span aria-hidden>→</span>
          </a>
          <p className="text-caption mt-4">No pitch. We'll go through your report and identify what to fix first.</p>
        </div>
      </section>
      ...
      <footer className="border-t border-ink-900/10 mt-12">
        <div className="max-w-canvas mx-auto px-6 lg:px-12 py-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="space-y-2">
            ...
            <p className="text-caption max-w-md">Built by Abhishek Rai. A free tool for B2B SaaS ops leaders.</p>
          </div>
          <div className="flex items-center gap-6 text-caption">
            <a href="https://abhishek-rai-1.netlify.app" target="_blank" rel="noopener noreferrer" className="hover:text-ember-600 transition-colors">Portfolio</a>
            <a href="mailto:abhishek.k0420@gmail.com" className="hover:text-ember-600 transition-colors">Contact</a>
          </div>
        </div>
      </footer>
    </main>
...
function TierBadge({ tier, label }: { tier: string; label: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-2 px-4 py-2 rounded-full border text-sm font-medium ${styles[tier] || styles.functional}`}>
